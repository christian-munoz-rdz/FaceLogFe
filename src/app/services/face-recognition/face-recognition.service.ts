...
export class FaceRecognitionService {
  ...
  public imageDimsDefault: any = {width: 224, height: 224};
  ...
  private subscribeEventDetection(
    ...
    this.observerFaceDetection$ = this.faceApiService.eventEmitterFaceDetection.subscribe(({faceDetections, isRecognitionLoaded}) => {
      console.log('DETECCION: ' + (faceDetections.length > 0 ? 'TRUE' : 'FALSE'));
  ...
  public startFaceRecognitionService(
    ...
    if (this.isFaceRecognitionServiceStopped()) {
      ...
      console.log('Recognition services already started');
    }
  ...
  public startAutoDetectFace(videoElement: ElementRef): void {
    if (this.isFaceRecognitionServiceStarted()) {
      if (this.isAutoDetectFaceStopped()) {
        ...
        console.log('Auto detect already started');
      }
    }
    else {
  ...
  private generateImageData(detection: any, videoElement: ElementRef): void {
    ...
    const imageSize = detection._height > detection._width ? detection._height + this.getPercentage(detection._height, 20) : detection._width + this.getPercentage(detection._width, 20);

    this.clearCanvas(canvasTmp);
    canvasTmp.getContext('2d').drawImage(videoElement.nativeElement, detection._x - xPercentage, detection._y - yPercentage, imageSize, imageSize, 0, 0, canvasTmp.width, canvasTmp.height);
  ...
  public imageDataToCanvas(imageData: number[][], canvas: any, imageDims: any = this.imageDimsDefault): void {
    let canvasTmp: any = document.createElement('canvas');
    ...
    canvas.getContext('2d').drawImage(canvasTmp, 0, 0, canvas.width, canvas.height);
